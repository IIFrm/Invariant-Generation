...
% However, if $P \wedge \neg C$ has a satisfying assignment $g$, 
% then $P(g)$ is true and $\mathcal{C}(g)$ is false 
% and hence g proves $\mathit{C}$ is not an invariant. 
...
% For an adequate invariant $\mathcal{I}$, $\mathcal{I}(b)$ should be false. 
% We call a state that must not be satisfied by an adequate invariant, such as b, a bad state. 

%Hence, given an incorrect candidate invariant and a decision procedure
%that can produce counterexamples, 
%the decision procedure can produce either a good state, a pair, or a bad state as a counterexample to refute the candidate.
...
Actually for an arbitrary state chain there are also two other possibilities we have not mentioned yet.
