...
\medskip\noindent
\textbf{Positive State, Negative State, Implication Pair}
tells the satisfiable of a state to the loop invariant.
%defines the label to a state.
These three concepts are originally introduced in \cite{sharma2014invariant}.
We present the idea and demonstrate the rules to category a state in the following.
...
Therefore, the state that must be satisfied by loop invariants is a positive state. 


Now consider inductive constraint~\ref{inv:loop}.
A pair of states $(s, t)$ is called implication pair
if $t = Body(s)$, and $s \models Cond$, then $(s \models Inv) \Rightarrow (t \models {Inv})$. 
Hence, for an invariant candidate $\mathcal{C}$, 
if there is a pair of states $(s, t)$ in which $t = Body(s)$ satisfying $(s \models \mathcal{C}) \wedge (t \models \neg \mathcal{C})$,
$\mathcal{C}$ is proved to be not a real invariant. 
