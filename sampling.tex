...
Random sampling provides us the initial set of samples to learn the first classifier. In this work, we have two ways to generate random samples. One is that we generate random values for each variable in $V$ based on its domain, based on a uniform probabilistic distribution over all value in its domain. The other is that we use an SMT solver~\cite{cite} to systematically generate valuations that satisfies $Pre$ as well as those that dissatisfies $Pre$. We remark that these two ways are complementary. On one hand, without using a solver, we may not be able to generate valuations which satisfy $Pre$ if $Pre$ is very restrictive (or dissatisfy $Pre$ if the negation of $Pre$ is very restrictive). On the other hand, using a solver often generates biased valuations. We remark that the cost of generating a random sampling is often eligible.
...
The concept of active learning and selective sampling refers to the approaches that aim at reducing the effort of collecting and labeling samples (i.e., marking a sample as either satisfying the invariant or not) by selecting
only the most informative samples. Selective sampling methods have been proposed for different learning algorithms~\cite{DBLP:conf/icml/OrabonaC11}. For instance, selective
sampling techniques for SVM-based learning have been proven effective in achieving a
high accuracy with fewer examples in many applications~\cite{DBLP:conf/icml/OrabonaC11,DBLP:conf/mm/TongC01,DBLP:journals/jmlr/TongK01}. The basic idea of selective sampling is that, given a classifier learned based on a set of samples, we calculate the most informative samples (according to the classifier) and label them so that the classifier can be improved the most. For instance, in the setting of SVM, it has been proved that the most informative samples are those right on the classification boundary~\cite{DBLP:conf/icml/OrabonaC11}.
...
a counter-example is provided by the verifier. We add the counter-example as new samples for the next learning iteration. Sampling through verification is necessary, which can be illustrated with the example shown in Figure~\ref{fig:running:example:sampling}. Using random sampling as presented previously, we only obtain valuations in the $\mathit{Positive}$ and $\mathit{NP}$ region but never any in the $\mathit{Negative}$ region. With selective sampling, we would converge to the half space separating $\mathit{Positive}$ and $\mathit{NP}$. As a result, we generate the candidate invariant $x - y \geq 0$. We would not however discover the half space $x - y \leq 16$. During invariant verification, formula (6) is as follows. \[
x - y \geq 0 \land \neg (x < y) \land \neg (x \geq y \land x \leq y + 16)
\] 
The formula is satisfied with a new valuation (a counter-example) $\{x \mapsto 17, y \mapsto 0\}$. According to our categorization, this valuation is added to $\mathit{Negative}$, which subsequently allow us to identify the candidate invariant separating $\mathit{Negative}$ from the rest: $x - y \geq 16$ and prove the Hoare triple. 
...
\noindent 
In the following, we define the four categorizes of samples formally. Let $\mathit{Sample}$ be the set of samples we obtain (through any of the above-discussed sampling methods). For each sample $s_0 \in \mathit{Sample}$, by executing the given program with initial variable valuation $s_0$, we can obtain a sequence of variable valuation $\langle s_0, s_1, \cdots, s_n \rangle$ where $s_i = \mathit{Body}^i(s_0)$ and $s_n \not \models \mathit{Cond}$ and $s_k \models \mathit{Cond}$ for all $0 \leq k < n$. In the following, we write $s \Rightarrow s'$ to denote that there exists a sample $s_0 \in \mathit{Sample}$ and a corresponding sequence of variable valuation $\langle s_0, \cdots, s_i, \cdots, s_j, \cdots s_n \rangle$ such that $s_i = s$ and $s_j = s'$ ($i \le j$). Next, we define the four disjoint sets of samples as follows.
\begin{align}
    & \mathit{CE}(\mathit{Sample}) & = & \{s | s_0 \models \mathit{Pre} \land s_0 \Rightarrow s \Rightarrow s_n \land s_n \not \models \mathit{Post}\} \\
    & \mathit{Positive}(\mathit{Sample}) & = & \{s | s_0 \models \mathit{Pre} \land s_0 \Rightarrow s \Rightarrow s_n \land s_n \models \mathit{Post}\}~~~~~~ \\
    & \mathit{Negative}(\mathit{Sample}) & = & \{s | s_0 \not \models \mathit{Pre} \land s_0 \Rightarrow s \Rightarrow s_n \land s_n \not \models Post\} \\
    & \mathit{NP}(\mathit{Sample}) & = & \{s | s_0 \not \models \mathit{Pre} \land s_0 \Rightarrow s \Rightarrow s_n \land s_n \models \mathit{Post}\}
\end{align}
    %& \mathit{CE}(\mathit{Sample}) & = & \{s | s \models \mathit{Pre} \land s \Rightarrow s' \land s' \not \models \mathit{Post}\} \\
    %& \mathit{Positive}(\mathit{Sample}) & = & \{s | s \models \mathit{Pre} \land s \models \mathit{Cond} \land s \Rightarrow s' \land s' \models \mathit{Post}\}~~~~~~ \\
    %& \mathit{Negative}(\mathit{Sample}) & = & \{s | s \not \models \mathit{Pre} \land s \Rightarrow s' \land s' \not \models Post\} \\
    %& \mathit{NP}(\mathit{Sample}) & = & \{s | s \not \models \mathit{Pre} \land s \Rightarrow s' \land s' \models \mathit{Post}\}

The following can be established immediately.
\begin{proposition}
1. If $\mathit{CE}(\mathit{Sample})$ is non-empty, the Hoare triple is disproved. \\
2. For all $s \in \mathit{Positive}(\mathit{Sample})$, $s \models \mathit{Inv}$ for all $\mathit{Inv}$ satisfying (1), (2) and (3). \\
3. For all $s \in \mathit{Negative}(\mathit{Sample})$, $s \not \models \mathit{Inv}$ for all $\mathit{Inv}$ satisfying (1), (2) and (3). \\
4. If $s \models \mathit{Inv}$ and $s \Rightarrow s'$, $s' \models \mathit{Inv}$ for all $\mathit{Inv}$ satisfying (1), (2) and (3). \hfill \qed
\end{proposition}
