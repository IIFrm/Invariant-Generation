...
a counter-example is provided by the verifier. We add the counter-example as new samples for the next learning iteration. To see how sampling through verification is necessary, assume that in the example shown in Figure~\ref{fig:running:example:sampling}, we only sample valuations in the $Positve$ and $NP$ region but never any in the $Negative$ region during random sampling. With selective sampling, we would converge to the half space separating $Positive$ and $NP$. As a result, we generate the candidate invariant $x-y \geq 0$. We would not however discover the half space $x - y \leq 16$. During invariant verification, formula (6) is as follows. \[
x - y \geq 0 \land \neg (x < y) \land \neg (x \geq y \land x \leq y + 16)
\] 
The formula is satisfied and a counterexample would be: $\{x \mapsto 17, y \mapsto 0\}$. According to our categorization, this valuation is added to $Negative$, which subsequently allow us to identify the candidate invariant separating $Negative$ from the rest: $x - y \geq 16$ and prove the Hoare triple.
