...
If the program cannot be proved, a counter-example will be provided.
...
Just say, we adopt symbolic execution to execute the program body 
...
\LL{Same here.}
After KLEE executes the loop-free program,
several path constraints are emitted as follows:
(As $\alpha \Rightarrow \beta \Longleftrightarrow \neg \big(\alpha \wedge \beta\big)$, 
KLEE \underline{emits} \LL{returns?} constraints with conjunctive forms, instead of inference rules.)
