...
Without considering the samples in $\mathit{NP}$, multiple classifiers located in the $\mathit{NP}$ region (e.g., $\mathit{x - y \leq 10}$, or $\mathit{x - y \leq 15}$) may be learned to perfectly classify $\mathit{Positive}$ and $\mathit{Negative}$. 
...
the classifier separating $\mathit{Positive}$ from $\mathit{Negative}$ and $\mathit{NP}$ would converge to $\mathit{NULL}$; %x - y \leq 0$; 
