...
\end{align}
In order to disprove the Hoare triple, we would like to find a valuation $s$ such that $s \in \mathit{Pre}$ and executing the loop until it terminates results in a valuation $s'$ such that $s' \not \in \mathit{Post}$. For simplicity, we assume that the loop always terminates and refer the readers to~\cite{Domagoj:FAC:2013,Hong:ASE:2015} %%\cite{acmcomm}
