...
based on iterations of sampling, learning and verification 
...
\begin{itemize}
    ...
    we \emph{actively} learn a classifier to classify the samples 
    using machine learning algorithms, e.g., SVM derivatives, 
    in the \textbf{Learning} stage (see Section~\ref{sec:classification}). 
    When the recently learnt classifier converges to the previously learnt ones, 
    we treat it as an invariant candidate and move to the verification step. 
    ...
    \LL{We prove the termination of the \emph{learning} stage if such an invariant exists.}
    \item 
    % (3) 
