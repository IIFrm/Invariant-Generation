...
\end{align}
For simplicity, we further assume that the given program always terminates and refer the readers to~\cite{} for extensive research on proving loop termination.
...
\begin{itemize}
    ...
    \item Set $\mathit{Negative}$ contains the set of valuations which do neither satisfy the precondition nor the postcondition afterwards. In Section~\ref{sec:sampling}, we prove that any $\mathit{Inv}$ which satisfies condition (1), (2) and (3), every valuation in $\mathit{Negative}$ must fail $\mathit{Inv}$.
