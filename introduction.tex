...
\begin{itemize}
    ...
    \item Set $\mathit{NP}$ contains all the valuations which have not appear in the above three sets. Section~\ref{sec:sampling} shows the satisfaction of these valuations to $\mathit{Inv}$ can not be decided according to the current knowledge. So they may or may not satisfy $\mathit{Inv}$.
\end{itemize}
%\begin{itemize}
...
It can be observed the classifier separating $\mathit{Negative}$ from the rest (i.e., $\mathit{x - y \leq 16}$) is the loop invariant that we are searching for. 
...
For instance, given the above-mentioned $\mathit{Positive}$ and $\mathit{Negative}$, 
...
With the new samples, a new classifier is then learnt.
\end{example}
...
the classifier separating $\mathit{Positive}$ from $\mathit{Negative}$ and $\mathit{NP}$ would be $\textsc{null}$; % $\emptyset$, %converge to $\varnothing$ %$\mathit{NULL}$; %x - y \leq 0$; 
