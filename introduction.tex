...
The overall workflow is shown in the Figure~\ref{fig:overview}. In the following, we use a loop example to illustrate how our approach works.
...
and the evaluation $\mathit{(x \mapsto x_0, ~y \mapsto y_0)}$ is written as $\mathit{(x_0, y_0)}$ in the following.
...
All the variable valuations $s_i~(0 \leq i \leq n)$ in such an evaluation sequence are divided into 
four sets $\mathit{CE}$ (Counter-Example), $\mathit{Positive}$, $\mathit{Negative}$, $\mathit{NP}$ (Negative or Positive).
...
$\mathit{\langle (1, 2)}, (11, 5) \rangle$, 
$\mathit{\langle (10, 1) \rangle}$ and $\mathit{\langle (100, 0) \rangle}$. 
